...
const HomeScreen = () => {
  ...
  const fetchDataPlants = async () => {
    try {
      const response = await fetch('http://192.168.1.6:3000/plants');
  ...
  const fetchDataPots = async () => {
    try {
      const response = await fetch('http://192.168.1.6:3000/pots');
      const json = await response.json();
      setpots(json);
    } catch (error) {
      console.error('Error fetching data:', error);
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View
        ...
        <Text style={styles.header}>Planta - toả sáng</Text>
